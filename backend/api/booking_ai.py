...
"""
AI routes — Booking conversational agent (LangGraph + Poly AI).

Routes
------
POST /booking/ai/chat     — run one agent turn; frontend sends full history
GET  /booking/ai/context  — returns live hotel state for the frontend to embed
...
from services.ai.booking_agent import run_agent

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/booking/ai", tags=["booking-ai"])
...
class AIChatRequest(BaseModel):
    ...
    hotel_context: Optional[str] = None     # pre-fetched from GET /booking/ai/context
...
async def _build_hotel_context(db: AsyncSession) -> str:
    ...
    for cat in sorted(cat_rooms):
        ...
        lines.append(
            f"  {cat:<10}  rooms={total}  available={free}"
...
async def get_hotel_context(db: AsyncSession = Depends(get_db)):
    ...
    and passes it as hotel_context with every /booking/ai/chat request.
    """
...
async def ai_chat(
    ...
    """
    Run one turn of the booking agent.
